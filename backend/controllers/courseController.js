...
exports.getCourses = asyncWrapper(async (req, res) => {
...
exports.getCourse = asyncWrapper(async (req, res) => {
...
exports.enrollCourse = asyncWrapper(async (req, res) => {
  const course = await Course.findById(req.params.id);
  if (!course || !course.isActive) {
    return res.status(404).json({ success: false, error: 'Course not found' });
  }
  const existing = await CourseEnrollment.findOne({ user: req.user.id, course: req.params.id }).lean();
  if (existing) {
  ...
  // Increment enrollment safely
  await Course.updateOne({ _id: req.params.id }, { $inc: { enrollmentCount: 1 } });
...
exports.getMyCourses = asyncWrapper(async (req, res) => {
...
exports.getFeaturedCourses = asyncWrapper(async (req, res) => {
